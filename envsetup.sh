...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ldroid_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $LDROID_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       LDROID_BUILD=
    fi
    export LDROID_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${LDROID_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the LDROID github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var LDROID_VERSION)
        ZIPFILE=L-Droid-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.ldroid.device=$LDROID_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $LDROID_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $LDROID_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/ldroid/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
